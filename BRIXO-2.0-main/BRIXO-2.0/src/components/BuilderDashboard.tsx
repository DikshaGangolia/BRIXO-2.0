...
export const BuilderDashboard: React.FC = () => {
  ...
  const triggerDeploy = async (platform: 'Vercel' | 'Netlify' | 'GitHub Pages') => {
    ...
    const domainSlug = config.businessName.toLowerCase().replace(/[^a-z0-9]/g, '');
    const platformDomain =
      platform === 'Vercel' ? 'vercel.app' :
      platform === 'Netlify' ? 'netlify.app' : 'github.io';

    const finalUrl = `https://${domainSlug}.${platformDomain}`;

...
console.log("activeProject:", activeProject);
  ...
  // Simulated deployment publisher pipeline
...
const response = await axios.put(
  `https://brixo-2-0.onrender.com/api/projects/publish/${activeProject.id}`,
  {},
  {
    headers: {
...
setDeployUrl(response.data.publishUrl);
} catch (error) {
              ...
              {(comp.fields.items || []).map((p: any, idx: number) => (
                ...
                >
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-between">
                    ...
                    <button
                      onClick={() => alert(`Simulated shopping checkout: Add ${p.name} to cart!`)}
                      className={`w-full py-1.5 border text-3xs font-semibold ${rounded}`}
                      style={{ borderColor: tokens.primaryColor, color: tokens.primaryColor }}
                    >
                      Add to cart
                    </button>
